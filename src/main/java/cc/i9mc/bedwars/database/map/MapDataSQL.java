package cc.i9mc.bedwars.database.map;
...
public class MapDataSQL implements Listener {
    ...
    public void onMessage(BukkitPubSubMessageEvent event) {
        if (event.getChannel().equals("MINIGame.BW." + IPUtil.getLocalIp())) {
            if (load) return;
            ...
            Bedwars.getInstance().mainThreadRunnable(() -> {
                ...
                Bedwars.getInstance().getGame().loadGame(mapData);
                Bukkit.getServer().getScheduler().scheduleAsyncRepeatingTask(Bedwars.getInstance(), () -> {
